...
SELECT p.IdPedido, p.nombreCliente, p.localidad, p.email, p.telefono, p.capacidadDeCarga, p.trochaDelEje, p.datosAdicionales, td.descripcion, tl.descripcion
FROM pedido p
INNER JOIN tipoDisposicion td ON td.IdTipoDisposicion = p.IdTipoDisposicion
INNER JOIN tipoLlanta tl ON tl.IdTipoLlanta = p.IdTipoLlanta
